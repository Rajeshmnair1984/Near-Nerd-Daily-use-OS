...
class DataService {
  ...
  private createLocalRecord<T extends object>(
    record: T,
  ...
  private updateLocalRecord<T extends { id: string }>(
    key: string,
    id: string,
    patch: Partial<T>,
  ): T {
  ...
  private deleteLocalRecord<T extends { id: string }>(
    key: string,
    id: string,
  ): void {
    this.saveLocalRecords(
  ...
  async getBills(): Promise<Bill[]> {
    ...
    if (!isSupabaseConfigured) {
      return this.applyOverdueDetection(
        this.getLocalRecords<Bill>("bills_cache"),
      );
    }
  ...
  async addLocation(location: CreateLocationInput): Promise<Location> {
    if (!isSupabaseConfigured) {
      ...
      this.saveLocalRecords("locations_cache", [
        ...this.getLocalRecords<Location>("locations_cache"),
        newLocation,
      ]);
    ...
    try {
      ...
      console.warn(
        "Failed to add location in Supabase, saving locally instead.",
        error,
      );
      const newLocation = this.createLocalRecord(location);
      this.saveLocalRecords("locations_cache", [
  ...
  async updateLocation(
    ...
    if (!isSupabaseConfigured) {
      const updatedLocation = this.updateLocalRecord<Location>(
        "locations_cache",
        id,
    ...
    try {
      ...
      console.warn(
        "Failed to update location in Supabase, updating local record instead.",
        error,
      );
      const updatedLocation = this.updateLocalRecord<Location>(
        "locations_cache",
  ...
  async deleteLocation(id: string): Promise<void> {
    ...
    try {
      ...
      console.warn(
        "Failed to delete location in Supabase, deleting local record instead.",
        error,
      );
      this.deleteLocalRecord<Location>("locations_cache", id);
      this.invalidateCache(false);
  ...
  async getDocuments(): Promise<DocumentRecord[]> {
    ...
    const cached =
      this.getFromLocalStorage<DocumentRecord[]>("documents_cache");
    if (cached) {
  ...
  async addBill(bill: CreateBillInput): Promise<Bill> {
    if (!isSupabaseConfigured) {
      ...
      this.saveLocalRecords("bills_cache", [
        ...this.getLocalRecords<Bill>("bills_cache"),
        newBill,
      ]);
      this.invalidateCache(false);
    ...
    try {
      ...
      console.warn(
        "Failed to add bill in Supabase, saving locally instead.",
        error,
      );
      const newBill = this.createLocalRecord(bill) as Bill;
      this.saveLocalRecords("bills_cache", [
  ...
  async addVendor(vendor: CreateVendorInput): Promise<Vendor> {
    if (!isSupabaseConfigured) {
      const newVendor = this.createLocalRecord({
        ...vendor,
        status: vendor.status || "Active",
      }) as Vendor;
      this.saveLocalRecords("vendors_cache", [
        ...this.getLocalRecords<Vendor>("vendors_cache"),
        newVendor,
      ]);
      this.invalidateCache(false);
    ...
    try {
      ...
      console.warn(
        "Failed to add vendor in Supabase, saving locally instead.",
        error,
      );
      const newVendor = this.createLocalRecord({
        ...vendor,
  ...
  async addDocument(document: CreateDocumentInput): Promise<DocumentRecord> {
    if (!isSupabaseConfigured) {
      const newDocument = this.createLocalRecord({
        ...document,
        status: document.status || "Active",
      }) as DocumentRecord;
      this.saveLocalRecords("documents_cache", [
        ...this.getLocalRecords<DocumentRecord>("documents_cache"),
        newDocument,
      ]);
      this.invalidateCache(false);
    ...
    try {
      ...
      console.warn(
        "Failed to add document in Supabase, saving locally instead.",
        error,
      );
      const newDocument = this.createLocalRecord({
        ...document,
  ...
  async updateDocument(
    ...
    if (!isSupabaseConfigured) {
      const updatedDocument = this.updateLocalRecord<DocumentRecord>(
        "documents_cache",
        id,
        document as Partial<DocumentRecord>,
      );
      this.invalidateCache(false);
    ...
    try {
      ...
      console.warn(
        "Failed to update document in Supabase, updating local record instead.",
        error,
      );
      const updatedDocument = this.updateLocalRecord<DocumentRecord>(
        "documents_cache",
  ...
  async deleteDocument(id: string): Promise<void> {
    ...
    try {
      ...
      console.warn(
        "Failed to delete document in Supabase, deleting local record instead.",
        error,
      );
      this.deleteLocalRecord<DocumentRecord>("documents_cache", id);
      this.invalidateCache(false);
  ...
  async updateVendor(id: string, vendor: UpdateVendorInput): Promise<Vendor> {
    if (!isSupabaseConfigured) {
      const updatedVendor = this.updateLocalRecord<Vendor>(
        "vendors_cache",
        id,
        vendor as Partial<Vendor>,
      );
      this.invalidateCache(false);
    ...
    try {
      ...
      console.warn(
        "Failed to update vendor in Supabase, updating local record instead.",
        error,
      );
      const updatedVendor = this.updateLocalRecord<Vendor>(
        "vendors_cache",
  ...
  async deleteVendor(id: string): Promise<void> {
    ...
    try {
      ...
      console.warn(
        "Failed to delete vendor in Supabase, deleting local record instead.",
        error,
      );
      this.deleteLocalRecord<Vendor>("vendors_cache", id);
      this.invalidateCache(false);
  ...
  async updateBillStatus(id: string, status: string): Promise<Bill> {
    if (!isSupabaseConfigured) {
      const updatedBill = this.updateLocalRecord<Bill>("bills_cache", id, {
        status,
      } as Partial<Bill>);
    ...
    try {
      ...
      if (updated) {
        ...
        if (status === "Paid" && updated.is_recurring) {
          ...
          if (nextError)
            console.error("Error creating next recurring bill:", nextError);
        }
        this.invalidateCache();
      ...
      console.warn(
        "Failed to update bill status in Supabase, updating local record instead.",
        error,
      );
      const updatedBill = this.updateLocalRecord<Bill>("bills_cache", id, {
        status,
  ...
  async updateBill(id: string, billData: UpdateBillInput): Promise<Bill> {
    if (!isSupabaseConfigured) {
      const updatedBill = this.updateLocalRecord<Bill>(
        "bills_cache",
        id,
        billData as Partial<Bill>,
      );
      this.invalidateCache(false);
    ...
    try {
      ...
      console.warn(
        "Failed to update bill in Supabase, updating local record instead.",
        error,
      );
      const updatedBill = this.updateLocalRecord<Bill>(
        "bills_cache",
  ...
  async deleteBill(id: string): Promise<void> {
    ...
    try {
      ...
      console.warn(
        "Failed to delete bill in Supabase, deleting local record instead.",
        error,
      );
      this.deleteLocalRecord<Bill>("bills_cache", id);
      this.invalidateCache(false);
  ...
  async getBillById(id: string): Promise<Bill | null> {
    if (!isSupabaseConfigured) {
      return (
        this.getLocalRecords<Bill>("bills_cache").find(
          (bill) => bill.id === id,
        ) || null
      );
    }
  ...
  async uploadDocumentFile(
    ...
    try {
      ...
      console.warn(
        "Failed to upload document to Supabase Storage, using local blob URL instead.",
        error,
      );
      onProgress?.(100);
      return URL.createObjectURL(file);
