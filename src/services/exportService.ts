...
export const exportService = {
  ...
  exportBillsToCSV(bills: Bill[], options: ExportOptions = {}) {
    const { filename = "bills", includeTimestamp = true } = options;
  ...
  exportBillsSummaryToCSV(bills: Bill[], options: ExportOptions = {}) {
    const { filename = "bills_summary", includeTimestamp = true } = options;
    ...
    const summaryLines = [
      ...
      ...Object.entries(categoryTotals).map(
        ([category, amount]) =>
          `${escapeCSVField(category)},${formatCurrency(amount)}`,
      ),
    ].join("\n");
  ...
  exportBillsToText(bills: Bill[], options: ExportOptions = {}) {
    const { filename = "bills_report", includeTimestamp = true } = options;
